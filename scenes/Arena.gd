...
var winner_packed = load("res://scenes/Winner_Board.tscn")
...
func _ready():
	...
	winner_board.get_child(0).visible = false
	add_child(winner_board)
...
func _process(delta):
	...
	if check_winner() != "none" and winner_in_effect:
		print(check_winner())
		$Winner_Board/Label.text = "WINNER\n" + check_winner()
		$Winner_Board/Label.visible = true
...
func _on_ball_collision(ball_collision):
	...
	if ball_collision.get_collider().name == "Shield":
		...
		#=# Call hitPlayer and pass the distance travelled by the hitting ball 
		collided_player.hitPlayer(ball_collision.get_travel())
		
		#=# Add balls hitting shield
		collided_player.balls_hit_shield += 1
		
		#==# Check how many balls hit and act accordingly
